...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if ("IntersectionObserver" in window && revealItems.length) {
        const revealImmediatelyIfNearViewport = (item) => {
            const rect = item.getBoundingClientRect();
            const viewportHeight = window.innerHeight || document.documentElement.clientHeight;
            const isNearViewport = rect.top <= viewportHeight * 1.15 && rect.bottom >= -viewportHeight * 0.15;
        ...
        revealItems.forEach((item, index) => {
            ...
            if (!revealImmediatelyIfNearViewport(item)) {
        ...
        const syncRevealItems = () => {
            revealItems.forEach((item) => {
                if (!item.classList.contains("is-visible") && revealImmediatelyIfNearViewport(item)) {
                    observer.unobserve(item);
                }
            });
        };

        window.addEventListener("resize", syncRevealItems, { passive: true });
        window.addEventListener("orientationchange", syncRevealItems, { passive: true });
        window.setTimeout(syncRevealItems, 180);
    } else {
    ...
    if (galleryImages.length) {
        ...
        lightbox.innerHTML = `
            ...
            <div class="memory-lightbox__dialog" role="dialog" aria-modal="true" aria-label="Enlarged gallery photo">
                <button class="memory-lightbox__nav memory-lightbox__nav--prev" type="button" aria-label="Show previous photo">&#8249;</button>
                <img class="memory-lightbox__image" src="" alt="">
                <button class="memory-lightbox__nav memory-lightbox__nav--next" type="button" aria-label="Show next photo">&#8250;</button>
            </div>
        `;
        ...
        const previousButton = lightbox.querySelector(".memory-lightbox__nav--prev");
        const nextButton = lightbox.querySelector(".memory-lightbox__nav--next");
        ...
        const showImageAtIndex = (index) => {
            ...
            activeTrigger = activeImage.closest(".memory-shot") || activeImage;

            const source = activeImage.currentSrc || activeImage.getAttribute("src") || activeImage.src;
        ...
        const showPrevious = () => {
            if (activeIndex >= 0) {
                showImageAtIndex(activeIndex - 1);
            }
        };
        ...
        galleryItems.forEach((image, index) => {
            const trigger = image.closest(".memory-shot") || image;

            trigger.tabIndex = 0;
            trigger.setAttribute("role", "button");
            trigger.setAttribute("aria-label", `${image.alt}. Open larger view`);
